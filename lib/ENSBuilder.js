...
class ENSBuilder {
  ...
  async bootstrap() {
    ...
    await waitToBeMined(this.deployer, this.ens.setOwner(utils.formatBytes32String(0), this.adminRegistrar.address));
  }

  async registerTLD(tld) {
    const label = utils.keccak256(utils.toUtf8Bytes(tld));
    const ethNode = utils.namehash(tld);
    await waitToBeMined(this.deployer, this.adminRegistrar.register(label, this.deployer.address, overrideOptions));
    await waitToBeMined(this.deployer, this.ens.setResolver(ethNode, this.resolver.address));
    this.registrars[tld] = await deployContract(this.deployer, FIFSRegistrar, [this.ens.address, ethNode]);
    await waitToBeMined(this.deployer, this.ens.setOwner(ethNode, this.registrars[tld].address));
  }
  ...
  async registerReverseRegistrar() {
    ...
    await waitToBeMined(this.deployer, this.registrars.reverse.register(labelHash, this.registrars['addr.reverse'].address, overrideOptions));
  }
  ...
  async registerDomain(label, domain) {
    ...
    await waitToBeMined(this.deployer, this.registrars[domain].register(labelHash, this.deployer.address, overrideOptions));
    await waitToBeMined(this.deployer, this.ens.setResolver(node, this.resolver.address));
    this.registrars[newDomain] = await deployContract(this.deployer, FIFSRegistrar, [this.ens.address, node]);
    await waitToBeMined(this.deployer, this.ens.setOwner(node, this.registrars[newDomain].address));
  ...
  async registerAddress(label, domain, address) {
    ...
    await waitToBeMined(this.deployer, this.registrars[domain].register(hashLabel, this.deployer.address, overrideOptions));
    await waitToBeMined(this.deployer, this.ens.setResolver(node, this.resolver.address));
    await waitToBeMined(this.deployer, this.resolver.setAddr(node, address));
  }

  async registerAddressWithReverse(label, domain, wallet) {
    await this.registerAddress(label, domain, wallet.address);
    await waitToBeMined(this.deployer, this.registrars['addr.reverse'].connect(wallet).setName(`${label}.${domain}`, overrideOptions));
  }
